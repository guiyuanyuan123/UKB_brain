...
f<-lm(EUR[,j] ~ EUR$f.21003.2.0 + EUR$f.25010.2.0)$residuals
EUR[,j][!is.na(EUR[,j])]=f
EUR_m0 <- EUR[EUR$f.22001.0.0==1,]
f<-lm(AFR[,j] ~ AFR$f.21003.2.0 + AFR$f.25010.2.0)$residuals
AFR[,j][!is.na(AFR[,j])]=f
AFR_m0 <- AFR[AFR$f.22001.0.0==1,]
f<-lm(EAS[,j] ~ EAS$f.21003.2.0 + EAS$f.25010.2.0)$residuals
EAS[,j][!is.na(EAS[,j])]=f
EAS_m0 <- EAS[EAS$f.22001.0.0==1,]
f<-lm(SAS[,j] ~ SAS$f.21003.2.0 + SAS$f.25010.2.0)$residuals
SAS[,j][!is.na(SAS[,j])]=f
...
write.table(zscore_EUR_AFR,"/data1/tmp/wangzhaobin/UKB/gyy_ukb/UKB_vol/ancestry_difference/tbv/tbv_1w_zscore_EUR_AFR",quote=F,row.names=F)
write.table(zscore_EUR_SAS,"/data1/tmp/wangzhaobin/UKB/gyy_ukb/UKB_vol/ancestry_difference/tbv/tbv_1w_zscore_EUR_SAS",quote=F,row.names=F)
write.table(zscore_EUR_EAS,"/data1/tmp/wangzhaobin/UKB/gyy_ukb/UKB_vol/ancestry_difference/tbv/tbv_1w_zscore_EUR_EAS",quote=F,row.names=F)
write.table(zscore_AFR_SAS,"/data1/tmp/wangzhaobin/UKB/gyy_ukb/UKB_vol/ancestry_difference/tbv/tbv_1w_zscore_AFR_SAS",quote=F,row.names=F)
write.table(zscore_AFR_EAS,"/data1/tmp/wangzhaobin/UKB/gyy_ukb/UKB_vol/ancestry_difference/tbv/tbv_1w_zscore_AFR_EAS",quote=F,row.names=F)
write.table(zscore_SAS_EAS,"/data1/tmp/wangzhaobin/UKB/gyy_ukb/UKB_vol/ancestry_difference/tbv/tbv_1w_zscore_SAS_EAS",quote=F,row.names=F)
