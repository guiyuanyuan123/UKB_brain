...
a=list.files("/data1/tmp/wangzhaobin/UKB/gyy_ukb/UKB_vol/assoc/female/txt")
dir=paste("/data1/tmp/wangzhaobin/UKB/gyy_ukb/UKB_vol/assoc/female/txt/",a,sep="")
n=length(dir)
for (i in 1:n){
 txt<-read.table(file=dir[i],stringsAsFactors=F)[-1,]
 colnames(txt)<- c("SNP", "B.A1", "B.A2","N","Z")
 MHC<-read.table("/data1/tmp/wangzhaobin/UKB/gyy_ukb/UKB_vol/snpinfo_1kg_nohm3",header=T,stringsAsFactors = F)[,c(2,4,5)]
...
file=paste("/data1/tmp/wangzhaobin/UKB/gyy_ukb/UKB_vol/heritability/female/all_autosomal/vol_all_",a[i],sep="")
